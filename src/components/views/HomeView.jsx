  ...
  return (
    <div className="recipe-home">
      ...
      <header className="recipe-home__title">
        <span className="recipe-home__mark"><DinnerBellIcon size={24} /></span>
        <span><h1>{t('home.indexTitle', 'Recipe index')}</h1><p>{t('home.indexSubtitle', 'A little book of ways into your kitchen.')}</p></span>
      </header>
      <section className="recipe-home__index">
        <h2>{t('home.beginTitle', 'Where do we begin?')}</h2>
        <div className="recipe-home__bell"><DecideCard recipes={recipes} cookStats={cookStats} onSelect={onSelectRecipe} /></div>
        {!isGuest && <>
        ...
        <HomeRow icon={<StatsIcon />} label={t('home.stats', 'Kitchen stats')} hint={t('home.statsHint', 'Favourites, repeats and forgotten gems')} onClick={onOpenStats} />
      </section>
    </div>
  )
}

function HomeRow({ icon, label, hint, onClick }) {
  return <button className="recipe-home__row" onClick={onClick}><span className="recipe-home__row-icon">{icon}</span><span><b>{label}</b><small>{hint}</small></span><span className="recipe-home__arrow" aria-hidden="true">›</span></button>
}
